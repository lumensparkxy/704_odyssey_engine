...
class DataGatherer:
    ...
    async def gather_data(self, intent_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Log the data gathering plan
        print(f"")
        print(f"📋 Data Gathering Plan:")
        print(f"   Sources to query: {', '.join(self.source_priority)}")
        print(f"   Research questions: {len(research_questions)}")
        print(
            f"   Key entities: {', '.join(key_entities[:5])}{'...' if len(key_entities) > 5 else ''}")
        print(f"")
        ...
        for source_type in self.source_priority:
            try:
                ...
                print(
                    f"{source_emoji} [{source_type.upper()}] Starting data collection...")

                if source_type == "internal_knowledge":
                    print(
                        f"   📍 Processing {len(research_questions)} research questions...")
                    ...
                    print(
                        f"   📍 Generating search queries from {len(research_questions)} questions...")
                    ...
                    search_data = gathered_data["sources"].get(
                        "google_search", {})
                    urls_to_scrape = search_data.get("urls", [])
                    print(f"   📍 Found {len(urls_to_scrape)} URLs to scrape")
                    scraped_data = await self._gather_web_scraping(urls_to_scrape)
                    gathered_data["sources"]["web_scraping"] = scraped_data

                # Log completion for each source
                self._log_source_completion(
                    source_type, gathered_data["sources"].get(source_type, {}))
        ...
        try:
            ...
            # Assess coverage
            print("   📈 Assessing research coverage...")
            ...
            sources_count = len([s for s in gathered_data["sources"].values(
            ) if not isinstance(s, dict) or "error" not in s])
            conflicts_count = len(gathered_data.get("conflicts", []))
            coverage = gathered_data.get("coverage_assessment", {}).get(
                "overall_coverage", "N/A")
            print(f"   📚 Sources processed: {sources_count}")
            print(f"   ⚠️  Conflicts found: {conflicts_count}")
    ...
    async def _gather_internal_knowledge(self, research_questions: List[str], key_entities: List[str]) -> Dict[str, Any]:
        ...
        if internal_data["responses"]:
            internal_data["confidence"] = sum(
                r["confidence"] for r in internal_data["responses"]) / len(internal_data["responses"])
    ...
    async def _gather_google_search(self, research_questions: List[str], key_entities: List[str]) -> Dict[str, Any]:
        ...
        # Configurable limit to prevent hanging
        for query in search_queries[:self.config.get("MAX_SEARCH_QUERIES", 3)]:
            ...
            try:
                ...
                search_data["results"].append(
                    {"response": "Search timed out", "error": "timeout"})
            except Exception as e:
                print(f"⚠️ Search error for query '{query}': {str(e)}")
                search_data["queries"].append(query)
                search_data["results"].append(
                    {"response": f"Search failed: {str(e)}", "error": str(e)})
    ...
    async def _follow_links(self, links: List[str], current_depth: int, scraped_data: Dict) -> List[Dict]:
        ...
        # Configurable limit links per page
        for link in links[:self.config.get("MAX_LINKS_PER_PAGE", 5)]:
            ...
            try:
                ...
                scraped_data["errors"].append(
                    {"url": link, "error": str(e), "depth": current_depth + 1})
    ...
    async def _extract_urls_from_response(self, response_text: str) -> List[str]:
        ...
        return urls[:self.config.get("MAX_URLS_FROM_RESPONSE", 10)]
    ...
    async def _assess_content_quality(self, content: str) -> float:
        ...
        try:
            ...
            score_match = re.search(
                r'"quality_score":\s*(\d+)', analysis.get("result", ""))
            if score_match:
    ...
    async def _assess_coverage(self, research_questions: List[str], consolidated_info: Dict) -> Dict[str, Any]:
        ...
        for question in research_questions:
            ...
            try:
                ...
                coverage_assessment["question_coverage"][question] = {
                    "coverage_score": 50, "gaps": ["Assessment failed"]}

        # Calculate overall coverage
        if coverage_assessment["question_coverage"]:
            scores = [q.get("coverage_score", 0)
                      for q in coverage_assessment["question_coverage"].values()]
    ...
    def _log_source_completion(self, source_type: str, source_data: dict) -> None:
        """Log completion summary for a data source."""
        if source_type == "internal_knowledge":
            responses = source_data.get("responses", [])
            confidence = source_data.get("confidence", 0)
            print(
                f"   ✅ Completed: {len(responses)} responses (confidence: {confidence:.0%})")
            ...
            print(
                f"   ✅ Completed: {len(queries)} queries → {len(results)} results, {len(urls)} URLs found")

        elif source_type == "web_scraping":
            pages = source_data.get("scraped_pages", [])
            errors = source_data.get("errors", [])
            print(f"   ✅ Completed: {len(pages)} pages scraped")
            if errors:
                print(f"   ⚠️  {len(errors)} pages failed to scrape")

        elif source_type == "provided_documents":
            docs = source_data.get("documents_processed", 0)
            print(f"   ✅ Completed: {docs} documents processed")

        else:
            print(f"   ✅ Completed: {source_type}")
